...
# Versão 7.0 - Adicionado Auto-Ajuste de parâmetros de rede
#
...
class CifEditorDialog(QDialog):
    ...
    def __init__(self, cif_content: str, initial_pattern: tuple, simulation_params: dict,
        ...
        # Dados experimentais para auto-ajuste
        self._experimental_data = experimental_data  # (two_theta, intensity) ou None
        self._wavelength = wavelength
        self._max_2theta = max_2theta
        ...
        try:
            ...
            # Auto-ajuste só disponível se tiver dados experimentais carregados
    ...
    def _init_ui(self):
        ...
        self.auto_fit_btn.clicked.connect(self._open_auto_fit)
        params_grid.addWidget(self.auto_fit_btn, 2, 4, 1, 2)
    ...
    def _open_auto_fit(self):
        ...
        try:
            from matfinder.tools.xrd.auto_fit_dialog import AutoFitDialog

            dialog = AutoFitDialog(
                cif_handler=self.cif_handler,
                experimental_data=self._experimental_data,
                wavelength=self._wavelength,
                max_2theta=self._max_2theta,
                parent=self
            )
            dialog.parametersAccepted.connect(self._apply_auto_fit_result)
            dialog.exec()
    ...
    def _apply_auto_fit_result(self, new_params: dict):
        """Aplica os parâmetros resultantes do auto-ajuste."""
        if not self.cif_handler:
            return

        try:
            self.cif_handler.update_lattice_params(
                a=new_params['a'], b=new_params['b'], c=new_params['c'],
                alpha=new_params['alpha'], beta=new_params['beta'], gamma=new_params['gamma']
            )
