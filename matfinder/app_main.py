...
class MaterialsApp(QMainWindow):
    ...
    def create_results_tableview_cod_rod(self):
        ...
        self.table_cod_rod.setHorizontalScrollMode(
            QAbstractItemView.ScrollMode.ScrollPerPixel
        )
        self.table_cod_rod.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        ...
        header_cod_rod.setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        header_cod_rod.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        header_cod_rod.setSectionResizeMode(2, QHeaderView.ResizeMode.Interactive)
        header_cod_rod.resizeSection(2, 100)
        header_cod_rod.setSectionResizeMode(3, QHeaderView.ResizeMode.Interactive)
        header_cod_rod.resizeSection(3, 120)
        header_cod_rod.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)
        header_cod_rod.setSectionResizeMode(5, QHeaderView.ResizeMode.Interactive)
        header_cod_rod.resizeSection(5, 200)
        ...
        header_cod_rod.setSectionResizeMode(8, QHeaderView.ResizeMode.ResizeToContents)
    ...
    def populate_results_display(self, standardized_results: list, db_choice: str):
        ...
        if is_cod_rod:
            ...
            for row, compound_data in enumerate(standardized_results):
                ...
                fav_item.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
                if is_fav:
                ...
                btn_link.setStyleSheet(
                    "QPushButton { text-decoration: underline; color: blue; "
                ...
                self.table_cod_rod.setItem(row, 4, QTableWidgetItem(compound_data.get("spacegroup_display", "N/A")))
                self.table_cod_rod.setItem(row, 5,
                                           QTableWidgetItem(compound_data.get("cell_parameters_display", "N/A")))
                self.table_cod_rod.setItem(row, 6, QTableWidgetItem(compound_data.get("volume_display", "N/A")))

                bib_text = compound_data.get("bibliography_display", "N/A")
                doi_text = compound_data.get("doi_display", "")
                if doi_text:
                    bib_text += (f" (DOI: <a href='https://doi.org/{doi_text}'>{doi_text})")
                ...
                self.table_cod_rod.setItem(row, 8, QTableWidgetItem(compound_data.get("year_display", "N/A")))
